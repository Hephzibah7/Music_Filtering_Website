<form method="post" action="result.php">
	<table>
		<tr><td><label><strong>Enter your query below</strong></label><br></td></tr>
		<tr><td><input type="text" name="query" maxlength="100" size="40"></td><td><input type="submit" value="Search"></td></tr>
	</table>				
</form>
...
try {
	...
	if ($_POST["query"]) {
		...
		if(in_array($components[0], $attribute_set) && in_array($components[1], $operator_set)) {
			...
			$total = count((array)$queryArray) / 4;
			$num_pages = ceil($total / 5);
			
			$query_str = "for \$song in subsequence(collection(\"musics\")/swarlipi, {$skip}, 5)
                  let \$title := \$song/INFO / TITLE / text ()
                  let \$taal := \$song/TAAL/TAAL_NAME/ text ()
                  let \$raag := \$song/RAAG/RAAG_NAME/ text ()
                  let \$sheet := \$song/SHEET/LINES 
